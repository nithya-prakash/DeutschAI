...
import { useQuizQuestion, useSubmitAttempt } from "@/hooks/use-quiz";
import type { QuizAttemptResult } from "@/types/api";

export function QuizCard({ topicId, onNext }: { topicId: string; onNext: () => void }) {
  const { data: question, isLoading } = useQuizQuestion(topicId);
  ...
  if (isLoading || !question) {
    return <p className="text-sm text-muted-foreground">Loading a question…</p>;
  }

  const onSubmit = () => {
    if (selected === null) return;
    submit.mutate(
      { question_id: question.id, selected_option_index: selected },
